...
import { registerStackDetectTool, registerComplexityScoreTool, registerGraphifyTools, } from "./project-tools.js";
...
export default function (pi) {
    ...
    pi.registerCommand("agent:run", {
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                ctx.ui.notify(result.success ? "Agent completed successfully" : "Agent failed", result.success ? "info" : "error");
                ...
                ctx.ui.setStatus("multi-agent", "");
                ctx.ui.notify(`Agent error: ${err instanceof Error ? err.message : String(err)}`, "error");
            }
        },
    });
    /**
    ...
    pi.registerCommand("agent:plan", {
        description: "Create a multi-agent execution plan: /agent:plan 'Build a REST API'",
        handler: async (args, ctx) => {
            if (!args.trim()) {
...
`;
    ...
    pi.registerTool({
        name: "list_agents",
        label: "List Agents",
        description: "List all available specialized agents in the multi-agent system. Use before delegating tasks.",
    ...
    pi.registerTool({
        name: "delegate_task",
        label: "Delegate Task",
        description: "Delegate a subtask to a specialized agent. The agent runs in an isolated child pi process and returns its output. Use list_agents first to see available agents.",
        promptSnippet: "Delegate a subtask to a named specialist agent running in a child process",
        promptGuidelines: [
        ...
        parameters: Type.Object({
            ...
            task: Type.String({
                description: "Clear, self-contained task description for the agent. Include relevant context from previous steps if needed.",
            }),
        }),
    ...
    pi.registerTool({
        name: "execute_plan",
        label: "Execute Plan",
        description: "Execute a structured multi-agent plan. Provide the plan as a numbered list in the 'plan' field. Each line: '1. agentName → task description'. The tasks run sequentially with accumulated context.",
        parameters: Type.Object({
            plan: Type.String({
                description: "Numbered plan. Each line: 'N. agentName → task description'",
            }),
        }),
        async execute(_toolCallId, params, signal, onUpdate, ctx) {
            ...
            const results = [];
            const execResult = await orchestrator.executePlan(parsed, signal, (agentName, task) => {
                onUpdate?.({
                    content: [
                        {
                            type: "text",
