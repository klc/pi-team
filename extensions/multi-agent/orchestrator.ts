...
import type { AgentPlan, AgentRunResult, PlanExecutionResult } from "./types.js";
...
export class Orchestrator {
   ...
   */
  parsePlan(planText: string): AgentPlan {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("#") || trimmed.startsWith("**")) {
        if (!summary) summary = trimmed.replace(/[#*]/g, "").trim();
        continue;
      }
  ...
  private summarizeResults(results: AgentRunResult[]): string {
