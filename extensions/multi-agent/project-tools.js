...
import { checkGraphify, queryGraph, findPath, explainNode, getGraphSummary, } from "./graphify.js";
function detect(value, confidence, source) {
    return { value, confidence, source };
}
export function registerStackDetectTool(pi) {
    pi.registerTool({
        name: "stack_detect",
        label: "Stack Detect",
        description: "Auto-detect the project's tech stack by scanning dependency files and configuration. " +
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (composerJson) {
                ...
                if (goMod.includes("github.com/gin-gonic/gin"))
                    backendFramework = detect("Gin", "high", "go.mod");
                else if (goMod.includes("github.com/labstack/echo"))
                    backendFramework = detect("Echo", "high", "go.mod");
                else if (goMod.includes("github.com/gofiber/fiber"))
                    backendFramework = detect("Fiber", "high", "go.mod");
            }
                ...
                if (deps.includes("django"))
                    backendFramework = detect("Django", "high", "deps");
                else if (deps.includes("fastapi"))
                    backendFramework = detect("FastAPI", "high", "deps");
                else if (deps.includes("flask"))
                    backendFramework = detect("Flask", "high", "deps");
            }
            else if (gemfile) {
                backend = detect("Ruby", "high", "Gemfile");
                if (gemfile.includes("rails"))
                    backendFramework = detect("Rails", "high", "Gemfile");
            }
            else if (allDeps["express"] || allDeps["fastify"] || allDeps["@nestjs/core"]) {
                backend = detect("Node.js", "high", "package.json");
                if (allDeps["@nestjs/core"])
                    backendFramework = detect("NestJS", "high", "package.json");
                else if (allDeps["fastify"])
                    backendFramework = detect("Fastify", "high", "package.json");
                else if (allDeps["express"])
                    backendFramework = detect("Express", "high", "package.json");
            }
            else if (cargoToml) {
            ...
            if (allDepStr.includes("mysql") || envStr.match(/mysql|DB_CONNECTION.*mysql/i))
                databases.push("MySQL");
            if (allDepStr.includes("postgres") || envStr.match(/postgres|pgsql/i))
                databases.push("PostgreSQL");
            if (allDepStr.includes("mongodb") || allDepStr.includes("mongoose"))
                databases.push("MongoDB");
            if (allDepStr.includes("sqlite") || envStr.includes("sqlite"))
                databases.push("SQLite");
            if (allDepStr.includes("redis") || envStr.includes("REDIS"))
                databases.push("Redis (cache/queue)");
            if (allDepStr.includes("clickhouse") || envStr.includes("CLICKHOUSE"))
                databases.push("ClickHouse (analytics)");
            if (allDepStr.includes("elasticsearch"))
            ...
            if (scripts["start"])
                devCommands.push("npm start");
            if (composerDeps["laravel/octane"])
                devCommands.push("php artisan octane:start --watch");
            if (composerDeps["laravel/framework"])
            ...
            if (backendFramework)
                lines.push(`- **Framework:** ${backendFramework.value}`);
            if (runtime)
                lines.push(`- **Runtime:** ${runtime.value}`);
            lines.push("");
            if (frontendFramework) {
                lines.push("## Frontend");
                lines.push(`- **Framework:** ${frontendFramework.value}`);
                if (metaFramework)
                    lines.push(`- **Meta-framework:** ${metaFramework.value}`);
                if (styling)
                    lines.push(`- **Styling:** ${styling.value}`);
                if (buildTool)
                    lines.push(`- **Build tool:** ${buildTool.value}`);
                if (ssr)
                    lines.push(`- **SSR:** ${ssr.value}`);
                if (tsconfig)
            ...
            const gaps = [];
            if (!backend)
                gaps.push("Backend language (no composer.json, go.mod, package.json backend deps, pyproject.toml, or Gemfile detected)");
            if (!backendFramework && backend)
                gaps.push("Backend framework (language detected but no recognized framework found)");
            if (!frontendFramework && packageJson)
                gaps.push("Frontend framework (package.json found but no recognized UI framework)");
            if (databases.length === 0)
                gaps.push("Database (check .env.example or docker-compose for DB config)");
            if (testFrameworks.length === 0)
                gaps.push("Test framework and test commands");
            if (gaps.length > 0) {
            ...
            if (verbose) {
                ...
                for (const [key, val] of Object.entries(detections)) {
                    if (val)
                        lines.push(`- **${key}:** ${val.value} (confidence: ${val.confidence}, source: ${val.source})`);
                }
            }
            return {
...
export function registerComplexityScoreTool(pi) {
    pi.registerTool({
        name: "complexity_score",
        label: "Complexity Score",
        description: "Estimate cyclomatic complexity of a source file or directory. " +
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (results.filter((r) => r.hotspots > 0).length === 0) {
                ...
                lines.push("", "## Recommendations", "", `Functions with complexity ≥ ${threshold} should be refactored.`, `Run with a specific file path to see detailed per-function breakdown.`);
            }
            return {
                content: [{ type: "text", text: lines.join("\n") }],
                details: {
...
function analyzeFileRaw(filePath, _threshold) {
    let source;
    try {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentFunc) {
            for (const kw of branchKeywords) {
                ...
                if (matches)
                    currentFunc.score += matches.length;
            }
...
// ── Graphify Tools ─────────────────────────────────────────────────
export function registerGraphifyTools(pi) {
    /**
     * graphify_check
    ...
    pi.registerTool({
        ...
        description: "Check if graphify is installed and a graph exists for this project. " +
        ...
        async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
            ...
            if (status.reportPath)
                text.push(`**Report:** \`${status.reportPath}\``);
            return {
    ...
    pi.registerTool({
        ...
        description: "Query the knowledge graph for context about concepts, files, or agents. " +
    ...
    pi.registerTool({
        ...
        description: "Get a plain-language explanation of a concept/node from the knowledge graph, " +
    ...
    pi.registerTool({
        ...
        description: "Get a summary of the Graph Report including God Nodes, Surprising Connections, and Communities. " +
...
// ── Helpers ────────────────────────────────────────────────────────
function analyzeFile(filePath, displayPath, threshold) {
    ...
    for (const fn of analysis.functions) {
        ...
        if (fn.score >= threshold)
            status = "🔴 Hotspot — refactor";
        else if (fn.score >= threshold * 0.7)
            status = "🟡 Warning";
        lines.push(`| \`${fn.name}\` | ${fn.line} | ${fn.score} | ${status} |`);
    }
    if (hotspots.length > 0) {
        lines.push("", "## Recommendation", "", `**${hotspots.length} function(s) exceed the complexity threshold of ${threshold}.**`, "", "These should be refactored. Each hotspot function should be broken into smaller, single-responsibility functions.");
    }
    else if (warnings.length > 0) {
        lines.push("", "## Note", "", `No hotspots found, but ${warnings.length} function(s) are approaching the threshold.`, "Consider simplifying them proactively.");
    }
    else {
